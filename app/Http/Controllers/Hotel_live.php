...
{    
		...
		public function __construct(){
			...
			$this->hotelbeds_key= '216ed78bd01d216eb6d4261178bc2eb8';  //$common_dataArr['hotelbeds_key'];
			$this->hotelbeds_secret='b14ebd32e7';  //$common_dataArr['hotelbeds_secret'];
	...
	$id=$id;
	 ...
	 if($hotelSearchData->product=='Plistbooking'){ 
		...
		//	dd($contentArr);
			...
			if ($hotelObj && $hotelObj->img_path) {
    $feature_image = 'http://photos.hotelbeds.com/giata/xxl/' . $hotelObj->img_path;
} else {
    $feature_image = 'http://photos.hotelbeds.com/giata/xxl/';
}
...
public function BookingCancellationFromAdmin($id){
	  // dd($id);
        ...
        if(@$bookingDetails->user_id){
		    ...
		    // dd($userDetail);
		    // dd((int)($userDetail->ample) , round($no_of_amples), (int)$bookingDetails->booked_ample);
		     if( @$userDetail->ample < round($no_of_amples)){
